...
sdat2 <- read_csv('../acs2016/csv_pus/ss16pusb.csv')
...
hdat <- read_csv('../acs2016/csv_hus/ss16husa.csv')
hdat <- hdat[,hVars]
for(nn in names(hdat)) if(is.character(hdat[[nn]])) hdat[[nn]] <- parse_integer(hdat[[nn]])

hdat2 <- read_csv('../acs2016/csv_hus/ss16husb.csv')
hdat2 <- hdat2[,hVars]
for(nn in names(hdat2)) if(is.character(hdat2[[nn]])) hdat2[[nn]] <- parse_integer(hdat2[[nn]])

hdat <- rbind(hdat,hdat2)
rm(hdat2);gc()

sdat <- cbind(sdat,hdat[match(sdat$SERIALNO,hdat$SERIALNO),])
rm(hdat); gc()
sdat <- sdat[,-which(colnames(sdat)=='SERIALNO')]
...
save(sdat,file='data/clusteringData1yr.RData')
