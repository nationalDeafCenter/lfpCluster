source('generalCode/readInData.r')
...
sdat <- readACS(year='18',oneORfive='one',pVars=pVars,hVars=hVars,pr=FALSE)

## sdat <- read_csv('../../data/byYear/ss18pusa.csv',col_types = cols(.default = col_character())) %>% type_convert()
...
## sdat2 <- read_csv('../../data/byYear/ss18pusb.csv',col_types = cols(.default = col_character())) %>% type_convert()
## #sdat <- read_csv('../data/acs5yr2016/ss16pusa.csv')
...
## hdat <- read_csv('../../data/byYear/ss18husa.csv',col_types = cols(.default = col_character())) %>% type_convert()
## hdat <- hdat[,hVars]
## for(nn in names(hdat)) if(is.character(hdat[[nn]])) hdat[[nn]] <- parse_integer(hdat[[nn]])

## hdat2 <- read_csv('../../data/byYear/ss18husb.csv',col_types = cols(.default = col_character())) %>% type_convert()
## hdat2 <- hdat2[,hVars]
